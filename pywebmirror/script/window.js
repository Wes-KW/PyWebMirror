...
const dom_mappings = [
    new DOMMapping(HTMLImageElement, "img[src]", "src", set_link),
    new DOMMapping(HTMLImageElement, "img[srcset]", "srcset", set_srcset),
    new DOMMapping(HTMLScriptElement, "script[src]", "src", set_link),
    new DOMMapping(HTMLEmbedElement, "embed[src]", "src", set_link),
    new DOMMapping(HTMLMediaElement, "audio[src], video[src]", "src", set_link),
    new DOMMapping(HTMLSourceElement, "source[src]", "src", set_link),
    new DOMMapping(HTMLSourceElement, "source[srcset]", "srcset", set_srcset),
    new DOMMapping(HTMLTrackElement, "track[src]", "src", set_link),
    new DOMMapping(HTMLIFrameElement, "iframe[src]", "src", set_link),
    new DOMMapping(HTMLLinkElement, "link[href]", "href", set_link),
    new DOMMapping(HTMLAnchorElement, "a[href]", "href", set_link),
    new DOMMapping(HTMLAreaElement, "area[href]", "href", set_link),
    new DOMMapping(HTMLFormElement, "form[action]", "action", set_link),
];
...
for (let dom_mapping of dom_mappings) {
    ...
    let desc = Object.getOwnPropertyDescriptor(target.prototype, attr);
    ...
    Object.defineProperty(target.prototype, attr, {
        ...
        configurable: true,
        get: function() {
            return desc_get.call(this);
        },
        set: function(value) {
            let old_value = desc_get.call(this);
            let new_value = func(`${target.name}.${attr}`, value);
            if (new_value !== old_value) {
                return desc_set.call(this, new_value);
            } else {
                return old_value;
            }
        }
